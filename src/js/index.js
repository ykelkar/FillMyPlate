...
const controlSearch = async () => {
    ...
    if (query) {
        ...
        try {
            ...
            console.log(`Problem occured searching for ${query}: ${error}`);
            clearLoader();
            recipeView.clearRecipe();
            recipeView.searchedRecipeNotFound(query);
        }
    }
}
...
elements.shopping.addEventListener('click', e => {
    ...
    if (e.target.matches('.shopping__delete, .shopping__delete *')) {
        ...
        if (state.list.items.length === 0) listView.clearItems();
...
// Restore likeed recipes on page load
...
elements.shoppingForm.addEventListener('submit', e => {
    ...
    let newItem = listView.getInput();
    listView.clearInput();

    let item = state.list.addItem(1, '', newItem);
    if (newItem.includes(',')) {
        newItem = newItem.split(',');
        item = state.list.addItem(...newItem);
    }
